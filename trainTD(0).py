...
def train_td0(num_episodes: int = 35000):
    ...
    for ep in range(1, num_episodes):
        if ep == 9000 or ep == 18000 or ep == 30000:
            alpha *= 0.1
        game = Backgammon()
        game.roll_dice()
        ...
        while not game.check_game_over():
            ...
            if best_move is None:
                ...
                loss = td_update(model, traces, x_t, x_tp1, alpha=alpha)
                losses.append(loss)
                continue
        ...
        x_T = get_board_features(game).to(device)
        loss = td_update(model, traces, x_T, z, alpha=alpha, gamma=1.0)
        losses.append(loss)
        if ep % 100 == 0:
    ...
    torch.save(model.state_dict(), "TD0v1e_35000.pt")
